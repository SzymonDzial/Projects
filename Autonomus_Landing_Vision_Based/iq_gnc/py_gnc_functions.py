...
########################################################################
    ...
    def set_heading(self, heading):
        ...
        heading = heading + self.correction_heading_g 
    ...
    def set_velocity(self,vx,vy,vz,psi,pitch,roll,yaw):
        self.set_heading(psi)

        #rospy.loginfo(
            #"Velocity set to x:{} y:{} z:{} origin frame".format(vx, vy, vz))

        
        self.vel.linear = Vector3(vx,vy,vz)
        self.vel.angular=Vector3(pitch,roll,yaw)
        self.local_vel_pub.publish(self.vel)

    def set_destination(self, x, y, z, psi):
        """This function is used to command the drone to fly to a waypoint. These waypoints should be specified in the 
        local reference frame. This is typically defined from the location the drone is launched. 
        Psi is counter clockwise rotation following the drone's reference frame defined by the x axis through the right side of the drone with the y 
        axis through the front of the drone.
